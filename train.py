...
import ray
import torch
from utils import get_config, get_scheduler, soft_update
import os
...
class SelfPlay:
    ...
    def epsilon_greedy(self, obs1_list, obs2_list, forecast1_list, forecast2_list):
        if not self.config.rl.train_search_for_both_sides: # Search for both players
            batch_obs_array = np.array(obs1_list + obs2_list).reshape(-1, self.config.env.obs_dim)
            batch_forecast_array = np.array(forecast1_list + forecast2_list).reshape(batch_obs_array.shape[0], -1) if self.config.env.use_forecast else None
            _, action_idx_batch = self.agent.batch_obs_search(batch_obs_array, batch_forecast_array)
        else: # Use payoff matrix of one player to search for another player
            obs1 = np.array(obs1_list).reshape(-1, self.config.env.obs_dim)
            obs2 = np.array(obs2_list).reshape(-1, self.config.env.obs_dim)
            forecast1 = np.array(forecast1_list).reshape(obs1.shape[0], -1) if self.config.env.use_forecast else None
            action_idx_1_batch, action_idx_2_batch = self.agent.both_sides_search(
                obs1=obs1,
                obs2=obs2,
                forecast1=forecast1,
            )
            action_idx_batch = torch.cat([action_idx_1_batch, action_idx_2_batch])
        ...
        # Split the actions
        action_batch = self.actions[action_idx_batch].cpu()
        bs = action_batch.shape[0]
        action_1_batch = action_batch[:bs//2]
        action_2_batch = action_batch[bs//2:]
        action_idx_1_batch = action_idx_batch[:bs//2]
        action_idx_2_batch = action_idx_batch[bs//2:]
        
        # Return results
        return action_1_batch, action_2_batch, action_idx_1_batch, action_idx_2_batch
    ...
    def run(self):
        ...
        while len(envs) > 0:
            ...
            action_1_batch, action_2_batch, action_idx_1_batch, action_idx_2_batch = self.epsilon_greedy(
                obs1_list=last_obs1_list,
                obs2_list=last_obs2_list,
                forecast1_list=last_forecast1_list,
                forecast2_list=last_forecast2_list
            )
...
class ReplayBuffer:
    ...
    def sample(self):
        if self.utd  > self.config.optimizer.UTD or self.samplable_indices.shape[0] < self.config.rl.n_warmup:
...
class Evaluator:
    ...
    def _evaluate(self, net: Network, player2: str):
        ...
        while len(envs) > 0:
            ...
            action1_array, _ = agent1.batch_obs_search(obs1_array, forecast1_array)
            if player2 != "baseline":
                action2_array = np.array([agent2.act(obs2) for obs2 in obs2_array])
            else:
                action2_array, _ = agent2.batch_obs_search(obs2_array, None)
    ...
    def evaluate(self):
        ...
        for player2 in ["baseline", "weak", "strong"]:
...
class Trainer:
    ...
    def train_on_observation(self, samples):
        samples = [s.to(self.device) if s is not None else s for s in samples]
        ...
        torch.nn.utils.clip_grad_norm_(self.net_policy.parameters(), self.config.optimizer.gradient_max_norm)
        self.optimizer.step()
    ...
    def latent_train(self, samples):
        samples = [s.to(self.device) if s is not None else s for s in samples]
        observations, forecast, _, _, _, _, _ = samples
        
        # Only train with first observation. Less worry about done condition.
        observations = observations[:, 0]
        forecast = forecast[:, 0] if forecast is not None else None
        
        # Sub-sample
        latent_indices = torch.randint(0, observations.shape[0], (self.config.optimizer.latent_batch_size,), device=self.device)
        observations = observations[latent_indices]
        forecast = forecast[latent_indices] if forecast is not None else None
        
        # Encode observation
        self.net_policy.eval()
        predicted_latent_states, predicted_state_values = self.net_policy.initial_inference(observations, forecast)
        self.net_policy.train()
        with torch.no_grad():
            target_net_predicted_latent_states, target_net_predicted_state_values = self.net_target.initial_inference(observations, forecast)
        rollouts = []

        # Imagination roll-outs
        for _ in range(self.config.rl.td_steps + 1):
            self.net_policy.eval()
            agent = HockeyAgent(nets=[self.net_policy], config=self.config, eval=False)
            player1_actions, player2_actions = agent.latent_both_sides_search(predicted_latent_states)
            self.net_policy.train()
            next_predicted_latent_states, predicted_rewards, _, next_predicted_state_values = self.net_policy.recurrent_inference(predicted_latent_states.detach(), 
                                                                                                                            player1_actions, 
                                                                                                                            player2_actions)
            with torch.no_grad():
                next_target_net_latent_states, _, _, next_target_net_state_values = self.net_target.recurrent_inference(target_net_predicted_latent_states,
                                                                                                                        player1_actions,
                                                                                                                        player2_actions)
                
            rollouts.append((predicted_rewards, predicted_state_values, target_net_predicted_state_values))
            
            predicted_latent_states = next_predicted_latent_states
            predicted_state_values = next_predicted_state_values
            target_net_predicted_latent_states = next_target_net_latent_states
            target_net_predicted_state_values = next_target_net_state_values
            
            torch.cuda.empty_cache()
        
        # Summarize results
        predicted_rewards = torch.cat([r for r, _, _ in rollouts], dim=-1)
        predicted_rewards = torch.cumsum(predicted_rewards, dim=-1)
        predicted_rewards = torch.clamp(predicted_rewards, -1, 1)
        predicted_state_values = torch.cat([v for _, v, _ in rollouts], dim=-1)
        target_values = torch.cat([t for _, _, t in rollouts], dim=-1)
        
        # Compute the target values
        lambda_ = self.config.rl.td_lambda
        mc_lambda_return = 0
        for n in range(1, self.config.rl.td_steps + 1):
            mc_lambda_rewards = predicted_rewards[:n].reshape(n, -1) # [n, bs]
            mc_lambda_discount = self.config.rl.discount ** torch.arange(n, device=self.device, dtype=mc_lambda_rewards.dtype).reshape(-1, 1) # [n ,1]
            mc_lambda_return = (mc_lambda_rewards * mc_lambda_discount).sum(dim=0) # [bs]
            td_lambda_bootstrap = (self.config.rl.discount ** n) * target_values[n] * (1 - predicted_rewards[n].abs()) # [bs]
            G_n = mc_lambda_return + td_lambda_bootstrap # [bs]
            if n < self.config.rl.td_steps:
                balance_weight = (1 - lambda_) * (lambda_ ** (n - 1))
            else:
                balance_weight = lambda_ ** (n - 1)
            mc_lambda_return += balance_weight * G_n
        td_target = mc_lambda_return.clip(-1+1e-5, 1-1e-5) # [bs, 1]. Clip since can be outside of range because of bootstrap.
    
        # Compute loss
        value_loss = self.config.optimizer.latent_value_loss_weight * F.smooth_l1_loss(predicted_state_values[0], td_target)
        
        # Latent back-prop
        self.optimizer.zero_grad()
        value_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.net_policy.parameters(), self.config.optimizer.latent_gradient_max_norm)
        self.optimizer.step()
            
        def grad_norm(module):
            return np.sqrt(sum([p.grad.norm().item()**2 for p in module.parameters() if p.grad is not None]))
        self.shared_memory.log_scalars.remote({
            "latent_grad_norm/state_value": grad_norm(self.net_policy.state_value),
            "latent_grad_norm/encoder": grad_norm(self.net_policy.encoder),
            "latent_grad_norm/reward": grad_norm(self.net_policy.reward),
            "latent_grad_norm/dynamic": grad_norm(self.net_policy.dynamic),
            "loss/latent_value_loss": value_loss.mean().item(),
        })
    ...
    def train(self):
        ...
        for _ in tqdm(range(self.config.rl.training_steps), mininterval=2):
            ...
            while (samples := ray.get(self.replay_buffer.sample.remote())) is None: # not enough samples in the replay buffer, sample more
                time.sleep(0.01)
                continue
            self.train_on_observation(samples)
            if self.config.rl.train_from_rollouts and self.training_steps > self.config.rl.start_train_from_rollouts:
                self.latent_train(samples)
            self.training_steps += 1
            self.scheduler.step()
...
def main():
    ...
    # Start data-collection on first GPU
    self_plays = []
    worker_epsilons = config.ray.epsilons
    num_workers = len(worker_epsilons)
    print(f"Starting workers with exploration epsilons {worker_epsilons}")
    for data_collector_rank, epsilon in enumerate(worker_epsilons):
        self_play = SelfPlay.options(namespace="main", num_cpus=1/num_workers, num_gpus=0.5/num_workers).remote(config, 
                                                                                                                                     replay_buffer, 
                                                                                                                                     shared_memory, 
                                                                                                                                     torch.device("cuda:0"),
                                                                                                                                     data_collector_rank,
    ...
    # Start training on second GPU, if exists
    trainer = Trainer(config, replay_buffer, shared_memory,  torch.device(f"cuda:{torch.cuda.device_count() - 1}"))
    trainer.train()
