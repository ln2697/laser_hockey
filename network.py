...
class StateValue(torch.nn.Module):
    ...
    def forward(self, latent_state):
        ...
        output = torch.sum(expert_outputs * gating_weights,
                           dim=-1)  # [batch_size, 1]
        self.gating_weights = gating_weights.squeeze()
...
class Network(torch.nn.Module):
    ...
    @torch.no_grad()
    def latent_policy(self, latent_state):
        ...
        rewards = rewards.reshape((bs, self.n_actions, self.n_actions))
        next_state_values = next_state_values.reshape((bs, self.n_actions, self.n_actions))
    ...
    def policy(self, obs, forecast):
        ...
        return self.latent_policy(latent_state)
